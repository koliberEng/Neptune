...
from   FlexLinkCoder      import CCrcProcessor, CLdpcProcessor, CPolarProcessor,  Interleaver
...
class CFlexLinkTransmitter():
    ...
    def SFBC_Encode(X0: np.complex_
        ...
        '''
        Scaling     = np.sqrt(0.5)    # = 0.717
        Port0First  = Scaling * X0
        Port0Second = Scaling * X1
        Port1First  = -1*Scaling*np.conj(X1)
        Port1Second = -1*Scaling*np.conj(X0)
    ...
    def InitializeResourceGrid(self
        ...
        for k in range(0, NumberSubcarriers):        # k is the subcarrier index
            ...
            # Convert the control bit into a BPSK symbol
            QamSymbol = np.complex64(2*CurrentControlBit - 1)
        ...
        if ControlInfo.NumberTxAntennas == 1:
            RefSignalP0 = np.complex64(-1)               # BPSK modulation 0/1 -> -1 + j0 / 1 + j0
        else:  
            RefSignalP0 = np.complex64(-1.414)           # Reference signals are BPSK modulated with 3dB boost 
                                                         # Thus bits 0/1 map to -1.414 + j0 / 1.414 + j0
        for k in range(0, NumberSubcarriers):            # k is the subcarrier index
            bRefSignalP0Opportunity = ((k-2) % 3 == 0)
            if bRefSignalP0Opportunity == True:
                ...
                RefSignalP0                                               *= -1         # Flip the polarity for the next iteration 
        ...
        # ----------------------------------
        RefSignalP1 = np.complex64(-1.414)           # Reference signals are BPSK modulated with 3dB boost 
                                                     # Thus bits 0/1 map to -1.414 + j0 / 1.414 + j0
        for k in range(0, NumberSubcarriers):        # k is the subcarrier index
            ...
            if bRefSignalP1Opportunity == True:
                ...
                RefSignalP1                                               *= -1         # Flip the polarity for the next iteration 
    ...
    def AddSignalField(self
            ...
            for l in range(1, NumberSignalFieldSymbols + 1):  # Remember, symbol l = 0 is reserved for the control information
                for k in range(0, NumberSubcarriers):
                    ...
                    bAvailableForSignalField = ResourceElementValue != EReType.Emtpy.value          and \
                                               ResourceElementValue != EReType.RefSignalPort0.value and \
                                               ResourceElementValue != EReType.Control.value 
            ...
            for l in range(1, NumberSignalFieldSymbols + 1):  # Remember, symbol l = 0 is reserved for the control information
                for k in range(0, NumberSubcarriers):
                    ...
                    bAvailableForSignalField = ResourceElementValueP0 != EReType.Emtpy.value          and \
                                               ...
                                               ResourceElementValueP0 != EReType.Control.value        and \
                                               ResourceElementValueP1 != EReType.RefSignalPort1
        ...
        # ------------------------------------------------------------------------
        # Step 4: Convert the Rate Matched Bit stream into BPSK or QPSK values and map them into the Resource Grid
        # ------------------------------------------------------------------------
        if self.ControlInformation.NumberTxAntennas == 1:
            StartBit = 0
            for l in range(1, NumberSignalFieldSymbols + 1):  # Remember, symbol l = 0 is reserved for the control information
        ...
        else:   # We have two antenna ports and we must use SFBC during the mapping process
            ...
            for l in range(1, NumberSignalFieldSymbols + 1):  # Remember, symbol l = 0 is reserved for the control information
                for k in range(0, NumberSubcarriers):
                    ...
                    if self.ResourceGridEnum[k, l, CFlexLinkTransmitter.AntPort0] == EReType.SignalField.value:
                        ...
                        if SignalFieldOpportunityCount % 2 == 1:
                            continue   # Wait for the second opportunity
                        ...
                        self.ResourceGrid[k-1, l, CFlexLinkTransmitter.AntPort0 ] = Port0First[0]
                        self.ResourceGrid[k-0, l, CFlexLinkTransmitter.AntPort0 ] = Port0Second[0]
                        self.ResourceGrid[k-1, l, CFlexLinkTransmitter.AntPort1 ] = Port1First[0]
                        self.ResourceGrid[k-0, l, CFlexLinkTransmitter.AntPort1 ] = Port1Second[0]
    ...
    def AddPayloadA(self
        ...
        for ListIndex in range(0, len(DataBlockList)):
            ...
            # -----------------------------------------------------------------------    
            InterleavingIndices            = Interleaver(self.SignalField.FEC_Mode, self.SignalField.CBS_A_Flag)
            ...
            while True:
                ...
                if BPS == 0:
                    self.ResourceGridEnum[k % self.NumSubcarriers, l] = EReType.Emtpy.value
            ...
            while True:
                ...
                # has been filled for subcarrier indices, k, that are larger than self.CenterSubcarrier, we need to 
                # make an adjustment
                # If k = 0, then this is the first subcarrier of the current resource block
                kVirtual = k
                if kVirtual > self.CenterSubcarrier: 
                    kVirtual = k -1 
                ...
                if bAvailableForPayloadA == True:
                    if self.ControlInformation.NumberTxAntennas == 1:
                        ...
                        if PayloadAOpportunityCount % 2 == 1:    # We want to take action every second resource element opportunity for PayloadA
                            # Increment the subcarrier and OFDM symbol index if necessary
                            k += 1                                                  # Proceed to next subcarrier
                            if k % self.NumSubcarriers == 0:                        # Proceed to next OFDM symbol
                                l += 1 
                            continue
    ...
    #                                                                                                                  #
    # ---------------------------------------------------------------------------------------------------------------- #
    def OfdmModulate(self) -> np.ndarray:
        '''
        This function OFDM modulates the QAM values in the resource grid
        '''
        NumberSubcarriers = self.ResourceGrid.shape[0]
        NumberOfdmSymbols = self.ResourceGrid.shape[1]
        ...
        for l in range(0, NumberOfdmSymbols):                       # l is the OFDM symbol index
            ...
            IfftInputBuffer[self.PosIfftIndices] = self.ResourceGrid[self.PosSubcarrierIndices, l]
            IfftInputBuffer[self.NegIfftIndices] = self.ResourceGrid[self.NegSubcarrierIndices, l]
            IfftOutputBuffer                     = np.fft.ifft(IfftInputBuffer)
            ...
            OfdmTxOutput[OutputSequenceIndex:OutputSequenceIndex+OfdmSymbolLength] = OfdmSymbol
    ...
    def BuildTxPacket(self) -> np.ndarray:
        ...
        OfdmTxOutput = self.OfdmModulate()

        # Concatenate all portions of the Tx Waveform
        TxOutput = np.hstack((AgcBurst.astype(np.complex64), PreambleA.astype(np.complex64), PreambleB.astype(np.complex64), OfdmTxOutput.astype(np.complex64)))
...
if __name__ == '__main__':
    ...
    if Test == 0:
        ...
        bOverSampling = False                          # Produce results at 20MHz or oversampled at 40MHz
                                                       # We use either a 1024 or 2048 size IFFT
        ...
        ControlInfo = CControlInformation(ReferenceSymbolPeriodicityIndex= 3      # 0/1/2/3 - [1, 3,  6, 12]
                                        ...
                                        , NumberTxAntennaPortFlag        = 0      # 0/1     - 1TX  / 2TX
        ...
        # > Build the packet
        # ------------------------------------------------------------------------------------------
        FlexLinkTransmitter.BuildTxWaveform
